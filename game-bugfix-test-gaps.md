...
| `8653a5f` | Obliterate spell rendering | **Complete:** `obliterate_render_test` drives production `sub_4CA720()` with a deterministic direction-table/effect fixture, verifying signed movement, mirrored effect coordinates, and target-neighborhood cleanup. Full spell dispatch, animation sequencing, and GPU presentation remain integration coverage. See [`rendering.md`](docs/rendering.md). | **Yes:** production effect-update path; complete spell and GPU rendering remain untested. |
| `1cc83e6` | Laser direction/assembly matching | **Complete:** `laser_direction_assembly_test` drives production `sub_499290()` and `sub_4992B0()` with signed direction slots and square geometry, verifying the 1-based assembly table layout and wrapped-edge visibility crossings. Full laser effect creation, animation, and GPU presentation remain integration coverage. See [`rendering.md`](docs/rendering.md). | **Yes:** production laser geometry helpers; complete laser rendering remains untested. |
