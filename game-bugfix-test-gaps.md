...
| `743f879` | Repeating-song jitter | Advance a deterministic audio scheduler through loop boundaries and assert gap-free track transitions. | **Yes:** scheduler integration test. |
| `3302bad` | Durability strings in inventory | Construct items at minimum, maximum, and damaged durability and assert displayed strings. | **Yes:** inventory display path. |
| `e5859eb` | God-mode spell changes and sage cheat | Execute both cheat commands against a spellbook fixture and assert the selected spell remains correct. | **Yes:** command dispatch. |
