...
int main(int argc, char **argv)
{
    ...
    if (argc != 4)
    {
        fprintf(stderr, "usage: %s !test.map !test.c gamefiles/app\n", argv[0]);
        return 2;
    }
    if (!file_is_nonempty(argv[1]))
    {
        fprintf(stderr, "missing or empty !test map: %s\n", argv[1]);
        return 1;
    }
    if (!source_contains_all(argv[2]))
    {
        fprintf(stderr, "!test source lacks expected EUD operations: %s\n", argv[2]);
        return 1;
    }
    ...
    puts("!test map/source and Nox gamefiles found; gameplay execution remains "
